...
class LoadRawOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        def publish(event: dict):
            import json
            redis_client.publish(f"ingestion:{self.ingestion_id}:events", json.dumps(event))
        ...
        try:
            with open(path, "r") as f:
                ...
                for row in reader:
                    ...
                    if len(batch_buffer) >= self.batch_size:
                        ...
                        conn.commit()
                        batch_buffer = []
                        publish({"status": "running", "task": "load_raw", "rows_loaded": rows_loaded})
            ...
            conn.commit()
            publish({"status": "running", "task": "load_raw", "rows_loaded": rows_loaded, "rows_rejected": rows_rejected})
